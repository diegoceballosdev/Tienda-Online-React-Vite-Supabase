...
export const getProducts = async (page: number) => {
	...
	const from = (page - 1) * itemsPerPage; // indica el primer item de la pagina: 0, 10, 20, ... posicion en array
	const to = from + itemsPerPage - 1; // indica el ultimo item de la pagina: 9, 19, 29, ... posicion en array
...
export const getFilteredProducts = async ({ page = 1, brands = [] }: { page: number; brands: string[] }) => {
    ...
    const from = (page - 1) * itemsPerPage; // indica el primer item de la pagina: 0, 12, 24, ... posicion en array
    const to = from + itemsPerPage - 1; // indica el ultimo item de la pagina: 11, 23, 35, ... posicion en array

    //usamos let (no const) porque esta query puede cambiar con el filtrado:
    let query = supabase
        .from('products')
        .select('*, variants(*)', { count: 'exact' })
        .order('created_at', { ascending: false })
    ...
    // agregamos el filtro comparando la marca seleccionada con el campo brand de la tabla products:
    if (brands.length > 0) {
...
export const getRandomProducts = async () => {
    ...
    // Seleccion de 4 productos al azar:
...
export const createProduct = async (productInput: ProductInput) => {
	try {
		...
		const { data: product, error: productError } = await supabase
            // No se insertan imagenes ni variantes aún, se hacen en pasos posteriores:
			.from('products')
			.insert({
		...
		const folderName = product.id; // Usar el ID del producto como nombre de la carpeta

        // se espera un arreglo de promesas:
		const uploadedImages = await Promise.all(  // map retorna un array de promesas

			productInput.images.map(async image => {

				const { data, error } = await supabase.storage // subir imagen 
					.from('product-images') //nombre del bucket
					.upload(`${folderName}/${product.id}-${image.name}`, image); // nombre del archivo dentro del bucket
				...
				return imageUrl; // retornar la URL pública de la imagen subida
			})
		);
...
export const deleteProduct = async (productId: string) => {
	...
	if (productImages.images.length > 0) {
		...
		const paths = productImages.images.map(image => {
			const fileName = image.split('/').pop(); // obtener el nombre del archivo desde la URL
...
export const updateProduct = async (
	...
	const imagesToDelete = existingImages.filter( // creamos un array con las imagenes que ya no estan en validImages
	...
	const existingVariants = productInput.variants.filter(v => v.id); // filtra todas las variantes que tengan id, es decir las existentes
	const newVariants = productInput.variants.filter(v => !v.id); // filtra todas las variantes que no tengan id, es decir las nuevas
	...
	const { error: deleteVariantsError } = await supabase
		...
		.delete()
		.eq('product_id', productId)
		.not( // filtra las variantes cuyo
			'id',
			'in',
			`(${currentVariantIds ? currentVariantIds.join(',') : 0})` // si el array esta vacio, se pasa un 0 para que no borre todas las variantes, sino un string con los ids separados por comas
            // (UIWE2030230-2230000, UIWE2030230-2230001, ...)
		);
