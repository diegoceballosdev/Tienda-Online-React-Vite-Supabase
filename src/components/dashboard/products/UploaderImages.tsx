	...
	const handleImageChange = (
		...
		if (e.target.files) {
			const newImages = Array.from(e.target.files).map(file => ({
				...
				previewUrl: URL.createObjectURL(file), // Crear una URL de vista previa para mostrar la imagen
			}));
			...
			setValue( // Conecta con el form y actualiza el valor de las imágenes
				'images',
				updatedImages.map(img => img.file || img.previewUrl) // Usar el archivo si está disponible, de lo contrario usar la URL de vista previa
			);
		}
	};
